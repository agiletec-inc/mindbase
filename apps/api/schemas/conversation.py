...
class ConversationCreate(BaseModel):
    ...
    source: str = Field(
        ..., description="Source of conversation (claude-code, chatgpt, etc.)"
    )
    source_conversation_id: Optional[str] = Field(
        None, description="Original conversation ID from source"
    )
    ...
    content: dict[str, Any] = Field(
        ..., description="Full conversation content (JSONB)"
    )
    metadata: Optional[dict[str, Any]] = Field(
        default_factory=dict, description="Custom metadata"
    )
    source_created_at: Optional[datetime] = Field(
        None, description="Original creation time"
    )
    project: Optional[str] = Field(
        None, description="Detected or assigned project name"
    )
            ...
            "example": {
                ...
                "content": {
                    "messages": [
                        {
                            "role": "user",
                            "content": "Implement PM Agent autonomous features",
                        },
                        {
                            "role": "assistant",
                            "content": "I'll implement Phase 0, 1, 2...",
                        },
                    ]
                },
                "metadata": {
                    "project": "superclaude",
                    "tags": ["pm-agent", "autonomous"],
                },
            }
        }
...
class AppSettings(BaseModel):
    ...
    repoRoot: str = Field(
        "~/github/mindbase", description="Path where make commands run"
    )
