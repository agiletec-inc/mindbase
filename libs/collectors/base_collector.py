...
class BaseCollector(ABC):
    ...
    def validate_conversation(self, conversation: Conversation) -> bool:
        ...
        for msg in conversation.messages:
            ...
            if msg.role not in ["user", "assistant", "system"]:
                logger.warning(
                    f"Unknown role '{msg.role}' in conversation {conversation.id}"
                )
                return False
    ...
    def filter_by_date(
        self, conversations: List[Conversation], since_date: Optional[datetime]
    ) -> List[Conversation]:
    ...
    def update_stats(self, conversations: List[Conversation]):
        ...
        self.stats["total_messages"] = sum(
            conv.get_message_count() for conv in conversations
        )
    ...
    def load_checkpoint(self, checkpoint_file: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if "last_sync" in checkpoint_data:
                self.last_sync_timestamp = self.normalize_timestamp(
                    checkpoint_data["last_sync"]
                )
