...
class ChatGPTCollector(BaseCollector):
    ...
    def collect(self, since_date: Optional[datetime] = None) -> List[Conversation]:
        ...
        for data_path in self.get_data_paths():
            ...
            elif "Local Storage" in str(data_path) or "Session Storage" in str(
                data_path
            ):
            ...
            for log_file in log_files:
                if (
                    "chat" in log_file.name.lower()
                    or "conversation" in log_file.name.lower()
                ):
    ...
    def _collect_from_indexeddb(
        self, indexeddb_path: Path, since_date: Optional[datetime]
    ) -> List[Conversation]:
        ...
        for storage_file in storage_files:
            if storage_file.suffix == ".db":
                ...
                conversations.extend(
                    self._collect_from_sqlite(storage_file, since_date)
                )
            else:
    ...
    def _collect_from_logs(
        self, log_file: Path, since_date: Optional[datetime]
    ) -> List[Conversation]:
        ...
        try:
            ...
            for line in lines:
                ...
                if "New conversation" in line or "Session started" in line:
                    ...
                    if current_messages:
                        conv = self._create_conversation_from_messages(
                            current_messages, current_conv_id
                        )
                        if conv:
                            conversations.append(conv)
                    ...
                    conv_id_match = re.search(
                        r"id[:\s]+([a-f0-9-]+)", line, re.IGNORECASE
                    )
                    current_conv_id = conv_id_match.group(1) if conv_id_match else None

                # Look for user messages
                elif any(
                    marker in line for marker in ["User:", "Human:", "Question:", ">>>"]
                ):
                ...
                # Look for assistant messages
                    ...
                    marker in line
                    for marker in ["Assistant:", "AI:", "ChatGPT:", "Response:", "<<<"]
                ):
    ...
    def _parse_database_row(self, row_dict: Dict[str, Any]) -> Optional[Conversation]:
        ...
        try:
            ...
            updated_at = self.normalize_timestamp(
                row_dict.get("updated_at") or row_dict.get("modified") or created_at
            )
    ...
    def _parse_json_conversation(self, data: Dict[str, Any]) -> Optional[Conversation]:
        ...
        try:
            ...
            conv_id = (
                data.get("id") or data.get("conversation_id") or data.get("thread_id")
            )
            ...
            updated_at = self.normalize_timestamp(
                data.get("update_time") or data.get("updated_at") or created_at
            )
    ...
    def _extract_conversations_from_text(self, text: str) -> List[Conversation]:
        ...
        for match in matches:
            try:
                ...
                if any(
                    key in data
                    for key in ["messages", "conversation", "chat", "mapping"]
                ):
                    conv = self._parse_json_conversation(data)
                    if conv:
                        conversations.append(conv)
    ...
    def _create_conversation_from_messages(
        self, messages: List[Message], conv_id: Optional[str] = None
