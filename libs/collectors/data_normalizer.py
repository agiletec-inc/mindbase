...
class DataNormalizer:
    ...
    def _normalize_conversation(
        ...
        try:
            ...
            if not normalized_messages:
                logger.debug(
                    f"Skipping conversation {conversation.id}: no valid messages after normalization"
                )
                return None
    ...
    def _get_message_hash(self, message: Message) -> str:
        ...
        content_str = (
            f"{message.role}:{message.content}:{message.timestamp.isoformat()}"
        )
        return hashlib.sha256(content_str.encode()).hexdigest()
    ...
    def validate_data_quality(
        ...
        if valid_conversations:
            quality_report["statistics"] = self._calculate_statistics(
                valid_conversations
            )
    ...
    def _validate_conversation_quality(self, conversation: Conversation) -> List[str]:
        ...
        for msg in conversation.messages:
            if len(msg.content) < 2:
                ...
                issues.append(
                    f"Suspiciously long message: {len(msg.content)} characters"
                )
