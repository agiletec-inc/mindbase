    ...
    def collect(self, since_date: Optional[datetime] = None) -> List[Conversation]:
        ...
        for data_path in self.get_data_paths():
            ...
            for json_file in json_files:
                if (
                    "conversation" in json_file.name.lower()
                    or "chat" in json_file.name.lower()
                ):
                    conversations = self._collect_from_json_export(
                        json_file, since_date
                    )
                    all_conversations.extend(conversations)
                ...
                try:
                    ...
                    for key, value in rows:
                        ...
                        if any(
                            ...
                            for keyword in ["conversation", "chat", "message", "thread"]
                        ):
    ...
    def _extract_conversations_from_text(self, text: str) -> List[Conversation]:
        ...
        for match in matches:
            try:
                ...
                if any(
                    key in data
                    for key in ["messages", "conversation", "chat", "thread"]
                ):
                    conv = self._parse_json_conversation(data)
                    if conv:
                        conversations.append(conv)
